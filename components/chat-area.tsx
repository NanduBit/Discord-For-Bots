...
export function ChatArea() {
  ...
  const messages = (apiMessages as any[]).map((m, i) => {
    ...
    return { id, name, text, time, avatarUrl };
  });

  const channelLabel =
    typeof channelId === "string" ? decodeURIComponent(channelId) : "channel";
    ...
    <section className="flex h-dvh flex-col">
      <div className="flex-1 overflow-y-auto">
        <div className="mx-auto max-w-5xl px-8 pb-6 pt-24">
          <h1
            id={channelLabel}
            className="text-balance text-3xl font-semibold tracking-tight text-slate-100"
          >
            #{channelLabel}
          </h1>

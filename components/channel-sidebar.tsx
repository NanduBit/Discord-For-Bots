...
export function ChannelSidebar() {
  ...
  // Identity (bottom user card) via SWR
  const { data: identity } = useSWR<Identity>("/api/identity", fetcher)
  const displayName = identity?.username ?? ""
  const userTag = identity ? `${identity.username}#${identity.discriminator}` : ""
  const avatarSrc =
  ...
  // Guilds
  const { data: guilds = [] } = useSWR<Guild[]>("/api/guilds", fetcher)
    ...
    <div className="relative flex h-dvh w-72 flex-col border-r border-white/5 bg-slate-950/80">
      <header className="px-4 pb-3 pt-4 text-sm font-medium text-slate-200">
        <div className="flex items-center gap-2">
          ...
          />
          <span className="truncate">{selectedGuild?.name ?? "Unknown Guild"}</span>
        </div>
      </header>
          ...
          return (
            <Link
              ...
              className={[
                ...
                isActive ? "bg-violet-500/5 ring-1 ring-inset ring-violet-400/20" : "hover:bg-slate-800/60",
              ].join(" ")}
              aria-current={isActive ? "page" : undefined}
            >
              <Hash className={["h-4 w-4", isActive ? "text-slate-200" : "text-slate-400"].join(" ")} />
              <span className={isActive ? "text-slate-200" : "text-slate-300"}>#{c.name}</span>
            </Link>
          )
        })}
      </nav>
      ...
      <div className="pointer-events-none absolute inset-x-3 bottom-3">
        <div className="pointer-events-auto flex items-center gap-3 rounded-lg border border-white/5 bg-slate-900/60 p-3">
          ...
          <div className="flex min-w-0 flex-col leading-tight">
            <span className="truncate text-sm text-slate-200">{displayName}</span>
